...
public final class NewsMapping {
	...
	public static boolean isNeedStyle=false;
	public static int timeout=20000;
	String name = "";
	/** 规则url链接片断 */
	List<String> urlkey = new ArrayList<String>(1);
	/** 编码 */
	String code = "";
	/** 板块标号 */
	...
	int blockindex = 0;
	/** 过滤id */
	List<String> filterblockkey = new ArrayList<String>(1);
	/** 过滤属性与值 */
	List<String> filterblockattr = new ArrayList<String>(0);
	 ...
	 */
	@Deprecated
	public final Element getTruncationBlockelement(Element obj) {
		if (blocklabel.size() == 0) return null;
		String begin = blocklabel.get(0);
		String end = blocklabel.get(1);
		String content = obj.html();
		int index1 = content.indexOf(begin);
		if (index1 == -1) return null;
		int index2 = content.indexOf(end, index1 + begin.length());
		if (index2 == -1) index2 = content.length();/* 如果第二个字符串没有找到则返回剩余部分 */
		String newContent = content.substring(index1, index2);
		return Jsoup.parse(newContent);
	}

	@Deprecated
	public final Element getBlockElement(Element obj) {
		if (obj == null) return null;
		int len = blockid.size();
		/* 按id进行精确检索 */
		for (int i = 0; i < len; i++) {
			Element link = obj.getElementById(blockid.get(i));
			if (link != null) return link;
		}
		/* 按class进行精确检索 */
		for (int i = 0; i < len; i++) {
			Elements links = obj.getElementsByClass(blockid.get(i));
			if (links.size() > 0) return links.get(0);
		}
		/* 按标签进行精确检索 */
		for (int i = 0; i < len; i++) {
			Elements links = obj.getElementsByTag(blockid.get(i));
			if (links.size() > 0) return links.get(0);
		}
		/* 如果仍然没有找到，则对所有单元进行模糊查找 */
		List<Element> newList = getElementsVague(blockid, obj);
		if (newList.size() > 0 && newList.size() < blockindex) return newList.get(blockindex);
		return null;
	}
	...
	public final Element getElement(Element obj) {
		...
		if (!isNeedScript) result = getRemoveScript(result);
		if (!isNeedStyle) result = getRemoveStyle(result);
	...
	/**
	 * 模糊查找link里所有标签，查出一个列表
	 * @param list List<String>
	 * @param link Element
	 * @return List<Element>
	 */
	@Deprecated
	private static final List<Element> getElementsVague(List<String> list, Element link) {
		List<Element> newList = new ArrayList<Element>();
		Elements newElements = link.getAllElements();
		for (int i = 0, len = newElements.size(); i < len; i++) {
			Element e = newElements.get(i);
			if (isElementVague(list, e)) newList.add(e);
		}
		return newList;
	}

	/**
	 * 模糊查找，判断标签按照id class 标签名是否属于关键组中
	 * @param list List<String>
	 * @param link Element
	 * @return boolean
	 */
	@Deprecated
	private static final boolean isElementVague(List<String> list, Element link) {
		String id = link.id();
		String cla = link.attr("class");
		String tag = link.tagName();
		for (int i = 0; i < list.size(); i++) {
			if (id != null && id.indexOf(list.get(i)) > -1) return true;
			if (cla != null && cla.indexOf(list.get(i)) > -1) return true;
			if (tag != null && tag.indexOf(list.get(i)) > -1) return true;
		}
		return false;
	}
