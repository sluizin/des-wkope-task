package des.wangku.operate.standard.subengineering.news;

import org.jsoup.Jsoup;
import org.jsoup.nodes.Document;
import org.jsoup.nodes.Element;
import org.jsoup.select.Elements;

import des.wangku.operate.standard.utls.UtilsReadURL;

public class NewsSource_Baidu extends AbstractNewsSource {



	public NewsSource_Baidu() {
		super(false, false, false, 20000);
	}
	public NewsSource_Baidu(boolean ishtml, boolean isscript, boolean isstyle, int timeout) {
		super(ishtml, isscript, isstyle, timeout);
	}

	@Override
	public void getNewsInforList(String key, int max) {
		loop: for (int p = 0; p < max; p += 10) {
			String urlHttp = "/ns?word=" + key + "&pn=" + p + "&tn=news";
			String content = UtilsReadURL.getRealContent("news.baidu.com", urlHttp, "utf-8");
			Document doc = Jsoup.parse(content);
			Elements links = doc.getElementsByClass("result");
			int len = links.size();
			for (int i = 0; i < len; i++) {
				Element link = links.get(i);
				putNewsInfor(key, link);
				if (newsList.size() >= max) break loop;
			}
		}
	}

	@Override
	protected void analysis(News_Infor e, Element eResult) {
		if (eResult == null) return;
		String title = "", href = "", source = "", datetime = "", picture = "", summary = "";
		Elements titleElement = eResult.getElementsByClass("c-title");
		if (titleElement.size() > 0) title = titleElement.text();
		/* 链接地址 */
		Elements as = titleElement.select("a[href]");
		if (as.size() > 0) href = as.get(0).attr("abs:href");
		e.href = href;
		/* 内容 */
		Elements summarys = eResult.getElementsByClass("c-summary");
		if (summarys.size() > 0) {
			Element esummary = summarys.get(0);
			Elements auths = esummary.getElementsByClass("c-author");
			if (auths.size() > 0) {
				String[] arr = auths.text().split(News_Consts.ACC_Nbsp + News_Consts.ACC_Nbsp);
				if (arr.length > 0) source = arr[0];
				if (arr.length > 1) datetime = arr[1].trim();
			}
			Elements es = esummary.getElementsByClass("c-span6");
			if (es.size() > 0) {
				Elements pics = es.get(0).getElementsByClass("c-img");
				if (pics.size() > 0) picture = pics.attr("abs:src");
				Elements content = esummary.getElementsByClass("c-span18");
				if (content.size() > 0) {
					content.select("p").remove();
					content.select("span").remove();
					summary = content.text();
				}
			} else {
				esummary.select("p").remove();
				esummary.select("span").remove();
				summary = summarys.text();
			}
		}
		e.insertAttrib( title, href , source , datetime ,  picture , summary);
	}

	@Override
	public String getSourceName() {
		return "百度";
	}

}
