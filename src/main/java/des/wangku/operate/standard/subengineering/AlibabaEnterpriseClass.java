...
public class AlibabaEnterpriseClass {
	...
	public void getArchives() {
		...
		try {
			...
			this.content = UtilsJsoup.getElementsClassFirst(document, "detail-info");
	...
	public void getContactinformation() {
		...
		try {
			...
			Element es = ess.first();
			if (es == null) return;
			this.linkman = UtilsJsoup.getElementsClassFirst(es, "membername");
		...
		public void readpict() {
			...
			try {
				...
				for (int i = 0; i < lis.size(); i++) {
					...
					Connection connect = Jsoup.connect(picurlString).headers(UtilsConsts.header_a);
